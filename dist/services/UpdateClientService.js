            ...
            return __generator(this, function (_b) {
                switch (_b.label) {
                    ...
                    case 1:
                        client = _b.sent();
                        if (!client)
                            throw new AppError_1.default('Client ID not found.', 404);
                        if (!email) return [3 /*break*/, 3];
                        return [4 /*yield*/, clientRepository.findOne({
                                email: email,
                            })];
                    case 2:
                        duplicatedEmail = _b.sent();
                        if (duplicatedEmail)
                            throw new AppError_1.default('This e-mail is already used.', 403);
                        client.email = email || client.email;
                        _b.label = 3;
                    case 3:
                        if (dataDeNascimento) {
                            parsedDate = new Date(dataDeNascimento);
